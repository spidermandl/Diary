...
public class RichTextEditView extends RichEditTextField implements 
	...
	private void setRepTextStatus(int position, CharSequence repText,int repStart,int repEnd){
		...
		while(true){
			if(position+spanStart-1<0||editState.get(position+spanStart-1)==PLAIN_TEXT//纯正文
					...
					||(editState.get(position+spanStart-1)==SUB_TITLE&&(position+spanStart-2<0||editState.get(position+spanStart-2)==PLAIN_TEXT)&&this.getText().charAt(position+spanStart-1)!=SUB_TITLE_LEFT)//第一个正标题 ‘【’符号判断
					)){
				/**
				 * 找下一个小标题起始点
				 */
	...
	private void setBefRepTextStatus(int position,char rightChar){
		if(position>=0&&editState.get(position)==SUB_TITLE&&rightChar==SUB_TITLE_LEFT){
			int i=position;
			boolean reverse=false;
			while(i>=0&&i<=position){
				if(this.getText().charAt(i)==SUB_TITLE_RIGHT&&!reverse){
					break;
				}else if(this.getText().charAt(i)==SUB_TITLE_LEFT||reverse){
					reverse=true;
	...
	private void setAftRepTextStatus(int position,char leftChar){
		...
		}else if((position-1)>=0&&editState.get(position-1)==SUB_TITLE&&leftChar!=SUB_TITLE_RIGHT && 
