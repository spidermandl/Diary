...
public class RichTextStringBuilder extends SpannableStringBuilder {
	...
	public SpannableStringBuilder replace(final int start, final int end,
		...
		mWatcher.beforeReplace(start, end, repText, repStart, repEnd);
		...
		if (start == end && start != length()) {
			...
			mWatcher.onBeforeInsert(start, repText, repStart, repEnd);
		} else if (repText.toString().equals("") && repStart == 0
				&& repEnd == 0) {
			action = DELETING;
		...
		} else if (length() != 0 && start == length() && end == length()) {
			...
			mWatcher.onBeforeAppend(end, repText, repStart, repEnd);
		} else {
			action = COMPOSING;
			...
			mWatcher.onBeforeCompose(start, end, repText, repStart, repEnd);
		}

		SpannableStringBuilder mString = super.replace(start, end, repText,
				repStart, repEnd); // This is
									// where
									// the
									// action
									// happens
		...
		mWatcher.afterReplace(start, end, repText, repStart, repEnd);
		mWatcher.onTextActionCursorMove(end);

		switch (action) {
		case COMPOSING:
			mWatcher.onAfterCompose(start, end, repText, repStart, repEnd);
			break;
		case APPENDING:
			mWatcher.onAfterAppend(end, repText, repStart, repEnd);
			break;
		case INSERTING:
			mWatcher.onAfterInsert(end, repText, repStart, repEnd);
			break;
		case DELETING:
	...
	public void removeSpan(Object what) {
		...
		mWatcher.onSpanRemoved(this, what, getSpanStart(what), getSpanEnd(what));
	}
